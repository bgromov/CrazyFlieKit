 ...
 */
public class BluetoothLink : NSObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    public func connect(_ address: String?, callback: @escaping (Bool) -> ()) {
...
//        print("Received value for characteristic: \(characteristic.uuid.uuidString), length: \(data.count)")
    ...
    public var bleUUID: UUID? {
        return crazyflie?.identifier
    }
    ...
    public func onStateUpdated(_ callback: @escaping (NSString) -> ()) {
    ...
    */
    public func sendPacket(_ packet: Data, callback: ((Bool) -> ())?) {
        self.btQueue.async {
